    ...
    if ([[self delegate] respondsToSelector:@selector(textFieldShouldSelect:)]) {
        if ([[self delegate] textFieldShouldSelect:self]) {
            if ([self applyFilterWithSearchQuery:self.text].count > 0) {
                self.text = [[[self applyFilterWithSearchQuery:self.text] objectAtIndex:0] objectForKey:@"title"];
                ...
                if ([[self delegate] respondsToSelector:@selector(textField:didEndEditingWithSelection:)]) {
                    [[self delegate] textField:self didEndEditingWithSelection:[NSDictionary dictionaryWithObjectsAndKeys:self.text, @"title", @"NEW", @"CustomObject", nil]];
                }
                else{
                    NSLog(@"<MPGTextField> WARNING: You have not implemented a method from MPGTextFieldDelegate that is called back when the user selects a search suggestion.");
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell textLabel] setText:[dataForRowAtIndexPath objectForKey:@"title"]];
    if ([dataForRowAtIndexPath objectForKey:@"address"] != nil) {
        [[cell detailTextLabel] setText:[dataForRowAtIndexPath objectForKey:@"address"]];
    }
...
{
    self.text = [[[self applyFilterWithSearchQuery:self.text] objectAtIndex:indexPath.row] objectForKey:@"title"];
...
{
    // NSPredicate *predicate = [NSPredicate predicateWithFormat:@"title BEGINSWITH[cd] %@", filter];
...
- (void)provideSuggestions
{
    ...
    if (tableViewController.tableView.superview == nil && [[self applyFilterWithSearchQuery:self.text] count] > 0) {
        ...
        tableViewController.tableView.layer.borderWidth = 0.0;
        tableViewController.tableView.layer.borderColor = [UIColor whiteColor].CGColor;
        tableViewController.tableView.layer.masksToBounds = YES;
        ...
        if (self.popoverSize.size.height == 0.0) {
            ...
            frameForPresentation.origin.y += self.frame.size.height + 3;
